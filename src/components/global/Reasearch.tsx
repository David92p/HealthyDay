...
type ResearchType = {
  ...
  type: "ingredients" | "recipe"
}

const Research:React.FC<ResearchType> = ({ title, type }) => {
  ...
  return (
    <div className='flex flex-col h-[200px] cursor-default bg-mygreen' style={{fontFamily: "Salsa"}}>
      ...
      <div className='flex justify-center items-center w-full h-1/3 relative'>
        <input ref={input} onChange={toggleResearch} type="text"placeholder={`Pasta pomodoro ...`} maxLength={30} className={`h-10 sm:w-full 2xl:w-3/4 bg-slate-100 text-mygreen text-3xl px-2 placeholder:text-2xl py-2 mx-auto sm:mx-4 rounded-md`}/>
        {
          isLoading 
          ? (
          ...
          : (
            ...
            ? (
              <div className='flex flex-col w-full 2xl:w-3/4 px-10 sm:px-6 2xl:px-2 pb-2 absolute top-full z-10 tracking-wider bg-mygreen text-slate-100 bg-opacity-75'>
                { 
                  searchList.map(el => {
                    return (
                      <div 
