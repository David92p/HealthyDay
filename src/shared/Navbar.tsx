...
const Navbar:React.FC = () => {
  ...
  return (
    <div className='flex flex-col w-full h-48 sm:h-64 2xl:h-[500px] relative cursor-default' style={{fontFamily: "Salsa"}}>
      ...
      <h1 style={{fontFamily: "Salsa"}} className='text-slate-100 absolute w-full text-end mt-4 pr-4 sm:pr-6 2xl:pr-10 text-xl sm:text-4xl opacity-100 2xl:cursor-pointer' onClick={() => navigate("/")}>Buongiorno Salute</h1>
      <div className='w-full flex justify-between items-end absolute bottom-0 2xl:pb-6'>
        <h1 className='text-mypink text-3xl sm:text-5xl 2xl:text-8xl pl-4 sm:pl-6 2xl:pl-10 pb-2 sm:pb-14 2xl:pb-10 tracking-wider'>La sana bontà che <br /> puoi trovare</h1>
        ...
        <div className='hidden sm:flex text-slate-100 text-2xl 2xl:text-4xl sm:justify-end gap-5 2xl:gap-8 absolute bottom-0 w-full pr-6 2xl:pr-10 pb-2'>
          ...
          <NavLink className='text-slate-100 cursor-pointer 2xl:hover:text-mypink' onClick={() => setMenu(false)} to='ingredienti'>Ingredienti</NavLink>
          ...
          <NavLink className='text-slate-100 cursor-pointer 2xl:hover:text-mypink' onClick={() => setMenu(false)} to='pianifica'>Pianifica</NavLink>
