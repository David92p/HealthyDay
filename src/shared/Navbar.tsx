...
const Navbar:React.FC = () => {
  ...
  return (
    <div className='flex flex-col w-full h-48 sm:h-64 2xl:h-[600px] relative' style={{fontFamily: "Salsa"}}>
      ...
      <div className='w-full flex justify-between items-end absolute bottom-0'>
        <h1 className='text-mypink text-3xl sm:text-4xl 2xl:text-6xl pl-4 2xl:pl-16 pb-2 sm:pb-14 2xl:pb-4 tracking-wider'>La sana bontà che <br /> puoi avere.</h1>
        ...
        <div className='hidden sm:flex text-slate-100 text-2xl 2xl:text-4xl sm:justify-end gap-5 2xl:gap-8 absolute bottom-0 w-full pr-10 2xl:pr-16 pb-2'>
          ...
          <NavLink className='text-slate-100 cursor-pointer 2xl:hover:text-mypink' onClick={() => setMenu(false)} to={''}>About</NavLink>
        </div>
      </div>
      {
        menu ? (
          <motion.div initial={{x:-300}} animate={{x:0}} transition={{delay: 0, duration: 0.5}} className='flex flex-col justify-center gap-10 py-14 pl-4 w-2/3 bg-mygreen opacity-90 absolute top-0 text-lg tracking-widest font-bold'>
